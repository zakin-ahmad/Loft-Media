...
function StatItem({ value, label, suffix, started, isLast }) {
  ...
  return (
    ...
    >
      <h2 className="text-3xl md:text-4xl font-extrabold text-[#C9A84C]">
        {count}
        {suffix}
      </h2>

      <p className="mt-2 text-[10px] md:text-xs tracking-[0.2em] uppercase text-white/40 text-center">
        {label}
      </p>
    </div>
  );
...
export default function StatsBar() {
  ...
  return (
    <section
      ...
      className="
      w-full
      bg-gradient-to-r from-[#080A0E] via-[#0D1018] to-[#080A0E]
      border-y border-white/10
    "
    >
      <div className="max-w-7xl py-8 mx-auto grid grid-cols-2 md:grid-cols-4">
