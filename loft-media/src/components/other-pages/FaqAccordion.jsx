      ...
      {items.map((item, i) => (
        <div key={i}>
          ...
          >
            ...
            <span className={`text-gold text-xl font-light transition-transform duration-300 flex-shrink-0 ml-4
              ${open === i ? 'rotate-45' : 'rotate-0'}`}>
              +
            </span>
          </button>
          <div className={`overflow-hidden transition-all duration-500 ${open === i ? 'max-h-48 pb-6' : 'max-h-0'}`}>
            <p className="text-white/40 text-[14px] font-light leading-[1.85]">{item.a}</p>
          </div>
        </div>
      ))}
    </div>
